...
For those that are intersted in working with the plug-in code, after cloning this respository to your local machine, open the project folder in Android Studio, or the Android code editor of your choosing.

The `WiFiDirectSDActivity` module contains the source code for a basic Unity Plug-in with peer-to-peer support leveraging Android's flavor of WiFi Direct Service Discovery, and a simple implimentation of Java Sockets. 

Within Android Studio, build the ARR file by selecting the module in the `Project` pane and then from the `Build` menu by select the 
`Make Module 'WiFiDirectPluginHarness.WiFiDirectSDActivity'` and the ARR file will get generated in the output folder mentioned above.
...
>
>The following interpretation is based on the research, and experimention conducted while creating this sample, however, it should not be considered an authoritive source for how WiFi Direct Service Discovery is implimented in the Android, or AOSP operating system.
...
```
For a full working sample of a Unity project that impliments this plug-in, please refer to our WiFi-Direct-Shared-Experience-Sample code sample project for Unity.
...
```
#### Passes: comma dilimeted string list of device names.
...
```
#### Passes: comma dilimeted string list of recently discovered services.
...
```
#### Passes: string containing commanad, and its arguments.
...
>
>Refer to the `Magic Leap Dev Sample`: WiFi-Direct-Share-Experience-Sample App for an example of one comma-delimited approach to sharing a number of common user interactions between devices
--- 
